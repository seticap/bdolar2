...
import {tokenServices, webSocketServices} from "@/app/services/socketService";
...
export default function Navbar2() {
    ...
    const [username, setUsername] = useState("");   // <- nombre dinámico
    ...
    useEffect(() => {
        ...
        // estado inicial del sidebar desde localStorage
        try {
    ...
    // Leer el usuario desde la cookie creada en el login
    useEffect(() => {
        if (typeof window === "undefined") return;

        const userFromCookie = cookieManager.getCookie("auth-user");
        if (userFromCookie) {
            setUsername(userFromCookie);
        } else {
            setUsername("Usuario"); // fallback si no existe
        }
    }, []);

    const logout = () => {
        cookieManager.deleteCookie("auth-token");
        cookieManager.deleteCookie("auth-user");
